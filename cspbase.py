...
class Variable:
    ...
    def cur_domain_size(self):
        """Return the size of the variables domain (without constructing
        list) """
        if self.is_assigned():
            return 1
        else:
...
class Constraint:
    ...
    def add_satisfying_tuples(self, tuples):
        """We specify the constraint by adding its complete list of
        satisfying tuples. """
        for x in tuples:
    ...
    def get_n_unasgn(self):
        """return the number of unassigned variables in the constraint's
        scope """
        n = 0
        for v in self.scope:
...
class BT:
    ...
    def __init__(self, csp):
        ...
        self.nPrunings = 0  # the number of value prunings during search
    ...
    def print_stats(self):
        print(
            "Search made {} variable assignments and pruned {} variable values"
                .format(self.nDecisions, self.nPrunings))
